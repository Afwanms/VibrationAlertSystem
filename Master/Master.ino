...
void debugAndSendData() {
  ...
  Serial.println(myData.targetID);

  Serial.print("Alert Type: ");

  switch(myData.alertType) {
  ...
  if (key) {
    if (!callMode) {
      if (key == 'A' || key == 'a') { 
        ...
        myData.alertType = ALERT_EMERGENCY;
        myData.targetID = 0;
        debugAndSendData();
      } 
      else if (key == 'C' || key == 'c') { 
      ...
      if (key >= '0' && key <= '9') {
        ...
        if (inputBuffer.length() > 0) {
          ...
          myData.alertType = ALERT_TEACHER;
          myData.targetID = targetStudent;
          debugAndSendData();
